...
python3 scripts/ops/entropy_scan.py --config configs/pipeline.yaml
...
if [ -f scripts/ops/git_auto_commit.py ]; then
  python3 scripts/ops/git_auto_commit.py --level ENTROPY --reason "scheduled entropy scan" || true
fi
