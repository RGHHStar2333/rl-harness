...
if [ -f "$state_path" ]; then
  ...
  if [ -n "$old_pid" ] && kill -0 "$old_pid" 2>/dev/null; then
    ...
    echo "先运行：python3 scripts/pause_training.py --status"
    exit 1
  fi
fi

echo "🚀 启动训练..."
nohup setsid python3 scripts/train.py --config configs/pipeline.yaml >> "$log_path" 2>&1 &
pid=$!

printf '{\n  "pid": %s,\n  "status": "running",\n  "started_at": %s,\n  "command": "python3 scripts/train.py --config configs/pipeline.yaml",\n  "cwd": "%s",\n  "log_path": "%s"\n}\n' "$pid" "$(date +%s)" "$(pwd)" "$log_path" > "$state_path"
